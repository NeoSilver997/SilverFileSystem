...
export class AuthManager {
  ...
  async getUserPermissions(userId) {
    try {
      const [rows] = await this.db.connection.query(
        'SELECT can_view_photos, can_view_music, can_view_videos FROM users WHERE id = ?',
        [userId]
      );
 ...
 * Now fetches current permissions from database to ensure real-time updates
 */
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // If user not found or error fetching permissions, use token permissions as fallback
...
export function requirePhotoPermission(req, res, next) {
  if (!req.user || !req.user.permissions || !req.user.permissions.photos) {
...
export function requireMusicPermission(req, res, next) {
  if (!req.user || !req.user.permissions || !req.user.permissions.music) {
...
export function requireVideoPermission(req, res, next) {
  if (!req.user || !req.user.permissions || !req.user.permissions.videos) {
