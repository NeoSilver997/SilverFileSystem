...
export class AuthManager {
  ...
  async initializeUsersTable() {
    try {
      ...
      const columnsToAdd = [
        { name: 'is_enabled', definition: 'BOOLEAN DEFAULT FALSE' },
        { name: 'is_admin', definition: 'BOOLEAN DEFAULT FALSE' },
        { name: 'can_view_photos', definition: 'BOOLEAN DEFAULT TRUE' },
        { name: 'can_view_music', definition: 'BOOLEAN DEFAULT TRUE' },
        { name: 'can_view_videos', definition: 'BOOLEAN DEFAULT TRUE' }
      ];

      for (const column of columnsToAdd) {
        try {
          await this.db.connection.execute(`ALTER TABLE users ADD COLUMN ${column.name} ${column.definition}`);
  ...
  async createDefaultUser() {
    try {
      ...
      if (rows[0].count === 0) {
        ...
        await this.db.connection.execute(
          'INSERT INTO users (username, password_hash, is_enabled, is_admin, can_view_photos, can_view_music, can_view_videos) VALUES (?, ?, TRUE, TRUE, TRUE, TRUE, TRUE)',
  ...
  async login(username, password) {
    try {
      ...
      const [users] = await this.db.connection.query(
        'SELECT id, username, password_hash, is_enabled, can_view_photos, can_view_music, can_view_videos FROM users WHERE username = ?',
        [username]
      );
      ...
      const token = jwt.sign(
        { 
          ...
          permissions: {
            photos: user.can_view_photos,
            music: user.can_view_music,
            videos: user.can_view_videos
          }
        },
        JWT_SECRET,
  ...
  async findOrCreateGoogleUser(profile) {
    try {
      ...
      const [existingUsers] = await this.db.connection.query(
        'SELECT id, username, email, profile_picture, is_enabled, can_view_photos, can_view_music, can_view_videos FROM users WHERE google_id = ?',
        [profile.id]
      );
      ...
      if (existingUsers.length > 0) {
        ...
        return {
          ...
          can_view_photos: user.can_view_photos,
          can_view_music: user.can_view_music,
          can_view_videos: user.can_view_videos
        };
      }
      ...
      const [result] = await this.db.connection.execute(
        `INSERT INTO users (username, email, google_id, profile_picture, auth_provider, is_enabled, can_view_photos, can_view_music, can_view_videos) 
         VALUES (?, ?, ?, ?, 'google', FALSE, TRUE, TRUE, TRUE)`,
    ...
    const token = jwt.sign(
      {
        ...
        email: user.email,
        permissions: {
          photos: user.can_view_photos !== false,
          music: user.can_view_music !== false,
          videos: user.can_view_videos !== false
        }
      },
      JWT_SECRET,
      { expiresIn: JWT_EXPIRY }
  ...
  async updateUserPermissions(userId, permissions) {
    try {
      ...
      if (permissions.photos !== undefined) {
        updates.push('can_view_photos = ?');
        values.push(permissions.photos);
      }
      if (permissions.music !== undefined) {
        updates.push('can_view_music = ?');
        values.push(permissions.music);
      }
      if (permissions.videos !== undefined) {
        updates.push('can_view_videos = ?');
        values.push(permissions.videos);
      }
  ...
  async getUserPermissions(userId) {
    try {
      const [rows] = await this.db.connection.query(
        'SELECT can_view_photos, can_view_music, can_view_videos, is_admin FROM users WHERE id = ?',
        [userId]
      );
      ...
      return {
        photos: rows[0].can_view_photos === 1,
        music: rows[0].can_view_music === 1,
        videos: rows[0].can_view_videos === 1,
 ...
 * Caches permissions to avoid DB hit on every request
 */
const permissionsCache = new Map();
const PERMISSIONS_CACHE_TTL = 30 * 1000; // 30 seconds

export function clearPermissionsCache(userId = null) {
  if (userId) {
...
export function authMiddleware(authManager) {
  return async (req, res, next) => {
    try {
      ...
      } else if (req.cookies && req.cookies.silverfs_token) {
        token = req.cookies.silverfs_token;
      }
      ...
      try {
        ...
        if (cached && (Date.now() - cached.time) < PERMISSIONS_CACHE_TTL) {
          decoded.permissions = cached.permissions;
        } else {
          try {
            const permissions = await authManager.getUserPermissions(decoded.id);
            decoded.permissions = permissions;
            permissionsCache.set(cacheKey, { permissions, time: Date.now() });
          } catch (err) {
            console.warn(`Failed to fetch permissions for user ${decoded.id}: ${err.message}`);
            return res.status(500).json({ error: 'Unable to verify current permissions' });
          }
        }
...
function hasPermission(req, permissionName) {
  return Boolean(req.user?.permissions?.[permissionName] || req.user?.permissions?.is_admin);
}

/**
 * Express middleware to check photo viewing permission
 * Must be used after authMiddleware
 */
export function requirePhotoPermission(req, res, next) {
  if (!hasPermission(req, 'photos')) {
    return res.status(403).json({ error: 'You do not have permission to view photos' });
  }
  next();
}

/**
 * Express middleware to check music viewing permission
 * Must be used after authMiddleware
 */
export function requireMusicPermission(req, res, next) {
  if (!hasPermission(req, 'music')) {
    return res.status(403).json({ error: 'You do not have permission to view music' });
  }
  next();
}

/**
 * Express middleware to check video viewing permission
 * Must be used after authMiddleware
 */
export function requireVideoPermission(req, res, next) {
  if (!hasPermission(req, 'videos')) {
    return res.status(403).json({ error: 'You do not have permission to view videos' });
  }
  next();
}
