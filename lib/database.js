...
export class DatabaseManager {
  ...
  async storeFilesBatch(files, scanId = null) {
    ...
    // Filter out files that already exist in DB (keep oldest)
    const paths = files.map(f => f.path);
    const pathChunks = [];
    for (let i = 0; i < paths.length; i += 1000) {
      pathChunks.push(paths.slice(i, i + 1000));
    }
    const existingPaths = new Set();
    for (const chunk of pathChunks) {
      const placeholders = chunk.map(() => '?').join(',');
      const [rows] = await this.connection.query(
        `SELECT path FROM scanned_files WHERE path IN (${placeholders})`,
        chunk
      );
  ...
  async computeQuickHash(fileId) {
    ...
    try {
      const [rows] = await this.connection.execute(
        'SELECT path, size, quick_hash FROM scanned_files WHERE id = ?',
        [fileId]
      );
      ...
      return { hash: null, size: row.size, path: row.path };
    } catch (err) {
      return null;
    }
  }
  ...
  async rebuildFolderTree() {
    ...
    try {
      // Clear existing data (DELETE instead of TRUNCATE to avoid FK issues)
      await this.connection.query('SET FOREIGN_KEY_CHECKS = 0');
      await this.connection.execute('TRUNCATE TABLE folder_tree');
      await this.connection.query('SET FOREIGN_KEY_CHECKS = 1');

      // Populate from scanned_files
      const [rows] = await this.connection.query(`
      ...
      // Add root drive entries FIRST
      const [driveAgg] = await this.connection.query(`
        SELECT SUBSTRING_INDEX(folder_id, '\\\\', 2) as drive_path,
               SUBSTRING(folder_id, 1, 3) as drive,
               COUNT(*) as file_count, SUM(size) as total_size
        FROM scanned_files
        WHERE folder_id IS NOT NULL AND folder_id != ''
        GROUP BY drive_path, drive
      `);
      const driveMap = new Map();
      for (const row of driveAgg) {
        const d = row.drive;
        if (!driveMap.has(d)) driveMap.set(d, { file_count: 0, total_size: 0 });
        driveMap.get(d).file_count += parseInt(row.file_count);
        driveMap.get(d).total_size += parseInt(row.total_size);
      }
      for (const [drive, data] of driveMap) {
        await this.connection.execute(`
          INSERT INTO folder_tree (folder_id, folder_name, file_count, total_size, recursive_count, recursive_size, depth)
          VALUES (?, ?, ?, ?, ?, ?, 1)
        `, [drive, drive, data.file_count, data.total_size, data.file_count, data.total_size]);
      }

      const BATCH = 1000;
      // Clean up any stale entries (e.g. old D:\ duplicates) before inserting
      await this.connection.query("DELETE FROM folder_tree WHERE folder_id LIKE '%\\\\' AND depth = 1");
      
      for (let i = 0; i < rows.length; i += BATCH) {
        const batch = rows.slice(i, i + BATCH);
        const values = batch.map(r => {
          const segments = r.folder_id.split('\\').filter(Boolean);
          const depth = segments.length;
          return [r.folder_id, segments[segments.length - 1] || r.folder_id, r.file_count, r.total_size, r.file_count, r.total_size, depth];
        });

        const placeholders = values.map(() => '(?, ?, ?, ?, ?, ?, ?)').join(',');
        const flat = values.flat();
        await this.connection.execute(
          `INSERT IGNORE INTO folder_tree (folder_id, folder_name, file_count, total_size, recursive_count, recursive_size, depth) VALUES ${placeholders}`,
          flat
        );
      }

      // Create missing intermediate folders
      const [allExisting] = await this.connection.query('SELECT folder_id FROM folder_tree');
      const existingSet = new Set(allExisting.map(r => r.folder_id));
      const [allFolders] = await this.connection.query('SELECT folder_id, depth FROM folder_tree WHERE depth > 1');
      const missing = [];
      for (const row of allFolders) {
        const segments = row.folder_id.split('\\').filter(Boolean);
        for (let i = 1; i < segments.length; i++) {
          const parentId = segments.slice(0, i).join('\\');
          if (!existingSet.has(parentId)) {
            existingSet.add(parentId);
            missing.push([parentId, segments[i - 1], 0, 0, 0, 0, i]);
          }
        }
      }
      if (missing.length > 0) {
        console.log(`  Creating ${missing.length} missing intermediate folders...`);
      ...
      // Populate parent_id from folder_id
      await this.connection.query(`
      ...
      // Set hidden flag for dot-prefix and system folders
      await this.connection.query(`
        UPDATE folder_tree SET hidden = 1 
        WHERE folder_name LIKE '.%' 
           OR folder_name IN ('$RECYCLE.BIN', '$SysReset', 'System Volume Information', 'Windows', 'ProgramData')
      `);

      // Calculate recursive values bottom-up from deepest to 1
