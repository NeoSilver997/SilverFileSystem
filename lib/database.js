  ...
  async storeMusicMetadata(fileId, metadata) {
    ...
    await this.connection.execute(
      ...
      [
        ...
        metadata.track?.albumArtist,
        metadata.track?.year,
        metadata.track?.genre?.[0], // Get first genre if array
        metadata.track?.trackNumber,
        metadata.track?.trackTotal,
        metadata.track?.diskNumber,
        metadata.track?.diskTotal,
