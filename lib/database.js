...
export class DatabaseManager {
    ...
    try {
      ...
      await this.connection.execute(`
        CREATE TABLE IF NOT EXISTS music_ratings (
          ...
          updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
          FOREIGN KEY (file_id) REFERENCES scanned_files(id) ON DELETE CASCADE,
          UNIQUE KEY unique_file_rating (file_id),
          INDEX idx_file_id (file_id),
   ...
   */
  async setMusicRating(fileId, rating) {
    if (!this.connection) {
      throw new Error('Database not connected');
    }
    ...
    await this.connection.execute(`
      INSERT INTO music_ratings (file_id, rating)
      VALUES (?, ?)
      ON DUPLICATE KEY UPDATE rating = ?, updated_at = CURRENT_TIMESTAMP
    `, [fileId, rating, rating]);

    return { success: true, fileId, rating };
  }
   ...
   */
  async recordMusicPlay(fileId) {
    if (!this.connection) {
      throw new Error('Database not connected');
    }

    await this.connection.execute(
      'INSERT INTO music_play_history (file_id) VALUES (?)',
      [fileId]
    );
