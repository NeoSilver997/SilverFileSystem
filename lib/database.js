...
export class DatabaseManager {
  ...
  extractFolderPath(filePath) {
    ...
    if (lastSlashIndex === -1) {
      // No folder separator found, file is in root or current directory
      return '';
    }
    ...
    // Ensure we don't return an empty string for root paths
    return folderPath || '/';
  }
  ...
  async findOrCreateFolder(fullPath, folderPath) {
    ...
    // Find parent folder ID if this is a nested folder (non-recursive approach)
    let parentFolderId = null;
    const parentPath = this.extractFolderPath(folderPath);
    if (parentPath && parentPath !== folderPath && parentPath !== '/' && parentPath !== '') {
      // Check if parent exists, but don't create it recursively to avoid deep recursion issues
      ...
      if (parentExists.length > 0) {
        parentFolderId = parentExists[0].id;
      }
      // If parent doesn't exist, we'll create this folder without a parent for now
      // The parent can be created and linked in a later iteration
    }
    
    // Create new folder with more defensive parameter handling
    try {
      const [result] = await this.connection.execute(
      ...
      // If insert fails, maybe another process created it concurrently
